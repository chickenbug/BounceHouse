...
@WebServlet(name = "RegisterRep", urlPatterns = {"/CustomerRepRegistration"})
public class RegisterRep extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		writer.println("<html>" +
							"<head>" +
								"<title>" +
									"Bouncehouse Emporium - Customer Rep Registration" +
								"</title>" +
							"</head>" +
							"<body>" +
								"<center>" +
		...
		if (request.getParameter("address") == null || request.getParameter("address").equals("")) {
			writer.println("Account creation failed: all fields are required, but some were left blank.<br>"
					+	"Please click <a href = \"adminFunctions.jsp\">here</a> to try again."
					+	"</body>"
					+	"</html>"
			);
			return;	
		...
		try {
			...
			if (error) {
				writer.println("Please click <a href = \"addRep.jsp\">here</a> to try again."
						+ 		"</body>"
						+ 	"</html>"
				);
			} else {
